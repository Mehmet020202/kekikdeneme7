...
class UgurFilm : MainAPI() {
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        try {
                ...
                if (iframe.contains(mainUrl)) {
                    ...
                    for ((kaynak, order) in kaynaklar) {
                        ...
                        try {
                            ...
                            Log.d("UGF", "Error processing source: ${e.message}")
                        }
                    }
                } else {
            ...
            if (!foundLinks) {
                mainDocument.select("div.video-player iframe, div.player iframe, iframe[src*='player'], iframe").forEach { iframe ->
                    val iframeSrc = fixUrlNull(iframe.attr("src")) ?: fixUrlNull(iframe.attr("data-src"))
                    if (iframeSrc != null) {
                        Log.d("UGF", "Found iframe: $iframeSrc")
            ...
            if (!foundLinks) {
                mainDocument.select("video source").forEach { source ->
                    val videoSrc = fixUrlNull(source.attr("src"))
                    if (videoSrc != null) {
                        Log.d("UGF", "Found video source: $videoSrc")
                        callback.invoke(
                            newExtractorLink(
                                source = "Direct Video",
                                name = "Direct Video",
            ...
            if (!foundLinks) {
                mainDocument.select("script").filter { it.data().contains("\"file\"") || it.data().contains("\"source\"") || it.data().contains("\"url\"") }.forEach { script ->
                    try {
                        ...
                        if (videoUrl != null && videoUrl.isNotEmpty()) {
                            Log.d("UGF", "Found JSON video URL: $videoUrl")
                            callback.invoke(
                                newExtractorLink(
                                    source = "JSON Video",
                                    name = "JSON Video",
                        ...
                        Log.d("UGF", "Error parsing JSON script: ${e.message}")
                    }
                }
            }
            ...
            Log.d("UGF", "Error in loadLinks: ${e.message}")
            return false
        }
    }
}
