...
class DiziKorea : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        try {
            ...
            // 1. Orijinal video-services yapısı
            ...
            if (!foundLinks) {
                document.select("div.video-player iframe, div.player iframe, iframe[src*='player'], iframe").forEach { iframe ->
                    ...
                    if (iframeSrc != null) {
                        Log.d("DZK", "Found iframe: $iframeSrc")
            ...
            if (!foundLinks) {
                document.select("video source").forEach { source ->
                    ...
                    if (videoSrc != null) {
                        Log.d("DZK", "Found video source: $videoSrc")
                        callback.invoke(
                            newExtractorLink(
                                source = "Direct Video",
                                name = "Direct Video",
                ...
                document.select("script").filter { it.data().contains("\"file\"") || it.data().contains("\"source\"") || it.data().contains("\"url\"") }.forEach { script ->
                    try {
                        ...
                        if (videoUrl != null && videoUrl.isNotEmpty()) {
                            Log.d("DZK", "Found JSON video URL: $videoUrl")
                            callback.invoke(
                                newExtractorLink(
                                    source = "JSON Video",
                                    name = "JSON Video",
                        ...
                        Log.d("DZK", "Error parsing JSON script: ${e.message}")
                    }
                }
            }
            ...
            Log.d("DZK", "Error in loadLinks: ${e.message}")
            return false
        }
    }
}
