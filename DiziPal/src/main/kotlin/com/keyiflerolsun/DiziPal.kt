...
// ! CloudFlare bypass
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        try {
            ...
            // 1. Orijinal series-player-container yapısı
            ...
            if (iframe != null) {
                ...
                try {
                    ...
                    Log.d("DZP", "Error processing iframe: ${e.message}")
                    loadExtractor(iframe, "${mainUrl}/", subtitleCallback, callback)
                    foundLinks = true
                }
            }
            ...
            if (!foundLinks) {
                document.select("div.video-player iframe, div.player iframe, iframe[src*='player'], iframe").forEach { iframe ->
                    ...
                    if (iframeSrc != null) {
                        Log.d("DZP", "Found iframe: $iframeSrc")
            ...
            if (!foundLinks) {
                document.select("video source").forEach { source ->
                    ...
                    if (videoSrc != null) {
                        Log.d("DZP", "Found video source: $videoSrc")
                        callback.invoke(
                            newExtractorLink(
                                source = "Direct Video",
                                name = "Direct Video",
                ...
                document.select("script").filter { it.data().contains("\"file\"") || it.data().contains("\"source\"") || it.data().contains("\"url\"") }.forEach { script ->
                    try {
                        ...
                        if (videoUrl != null && videoUrl.isNotEmpty()) {
                            Log.d("DZP", "Found JSON video URL: $videoUrl")
                            callback.invoke(
                                newExtractorLink(
                                    source = "JSON Video",
                                    name = "JSON Video",
                        ...
                        Log.d("DZP", "Error parsing JSON script: ${e.message}")
                    }
                }
            }
            ...
            Log.d("DZP", "Error in loadLinks: ${e.message}")
            return false
        }
    }
        companion object {
