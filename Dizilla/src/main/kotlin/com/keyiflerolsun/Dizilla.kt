...
class Dizilla : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        try {
            ...
            // 1. Orijinal __NEXT_DATA__ yapısı
            val script = document.selectFirst("script#__NEXT_DATA__")?.data()
            if (script != null) {
                try {
                    val objectMapper = ObjectMapper().registerModule(KotlinModule.Builder().build())
                    ...
                    Log.d("DZL", "Error processing __NEXT_DATA__: ${e.message}")
                }
            }
            ...
            if (!foundLinks) {
                document.select("div.video-player iframe, div.player iframe, iframe[src*='player'], iframe").forEach { iframe ->
                    ...
                    if (iframeSrc != null) {
                        Log.d("DZL", "Found iframe: $iframeSrc")
            ...
            if (!foundLinks) {
                document.select("video source").forEach { source ->
                    ...
                    if (videoSrc != null) {
                        Log.d("DZL", "Found video source: $videoSrc")
                        callback.invoke(
                            newExtractorLink(
                                source = "Direct Video",
                                name = "Direct Video",
                ...
                document.select("script").filter { it.data().contains("\"file\"") || it.data().contains("\"source\"") || it.data().contains("\"url\"") }.forEach { script ->
                    try {
                        ...
                        if (videoUrl != null && videoUrl.isNotEmpty()) {
                            Log.d("DZL", "Found JSON video URL: $videoUrl")
                            callback.invoke(
                                newExtractorLink(
                                    source = "JSON Video",
                                    name = "JSON Video",
                        ...
                        Log.d("DZL", "Error parsing JSON script: ${e.message}")
                    }
                }
            }
            ...
            Log.d("DZL", "Error in loadLinks: ${e.message}")
            return false
        }
    }
